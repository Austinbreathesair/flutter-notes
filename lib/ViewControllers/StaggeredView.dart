...
class _StaggeredGridPageState extends State<StaggeredGridPage> {
  ...
  MyStaggeredTile _tileGenerator(int i) {
    return MyStaggeredTile( Note(
      ...
      _allNotesInQueryResult[i]["category"] == null ? "" : utf8.decode(_allNotesInQueryResult[i]["category"]),
      DateTime.fromMicrosecondsSinceEpoch(_allNotesInQueryResult[i]["date_created"] * 1000),
      DateTime.fromMicrosecondsSinceEpoch(_allNotesInQueryResult[i]["date_last_edited"] * 1000),
      Color(_allNotesInQueryResult[i]["note_colour"]))
      );
  }

  void retrieveAllNotesFromDatabase() {
    var _testData = noteDB.selectedAllNotes();
