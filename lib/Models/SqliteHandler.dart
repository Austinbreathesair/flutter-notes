...
class NotesDBHandler {
  ...
  initDB() async {
    ...
    Database dbConnection = await openDatabase(
      ...
      print("executing create query from onCreate");
  ...
  Future<int> insertNote(Note note, bool isNew) async {
    ...
    //Insert new notes into correct table
    ...
    if (isNew) {
      // get latest that isn't archived, limit by 1
  ...
  Future<List<Map<String,dynamic>>> selectedAllNotes() async {
    final Database db = await database;
    // query all notes sorted by last_edited date
